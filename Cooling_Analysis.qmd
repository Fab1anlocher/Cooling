...
cooling_sales <- sales_data %>%
  ...
  # Nur relevante Kategorien behalten (keine PC-Wasserkühlung, Luftreiniger, Gaming, Heizlüfter, etc.)
  filter(
...
# Hauptvisualisierung wird in Abschnitt 3.3 gezeigt (normalisierte Werte)
# Hier nur zur Referenz die Rohdaten
p1 <- ggplot(analysis_data, aes(x = Temperatur, y = Anzahl_Verkaeufe)) +
  ...
  labs(
    title = "Verkäufe von Kühlprodukten nach Temperatur (Absolute Zahlen)",
    subtitle = "Hinweis: Größere Städte haben naturgemäß mehr absolute Verkäufe",
    x = "Durchschnittstemperatur des wärmsten Monats (°C)",
    y = "Anzahl Verkäufe (absolut)",
    size = "Bevölkerung",
    color = "Stadt"
  ) +
  theme_minimal() +
...
# Plot wird nicht direkt angezeigt, nur für Vergleich gespeichert
```

## 3.3 Normalisierung nach Bevölkerungsgröße

Um faire Vergleiche zwischen Städten unterschiedlicher Größe zu ermöglichen, berechnen wir **Verkäufe pro 5,000 Einwohner**.

```{r normalized-sales}
# Verkäufe pro 5.000 Einwohner
analysis_data <- analysis_data %>%
  mutate(
    Verkaeufe_pro_5k = (Anzahl_Verkaeufe / Bevoelkerung) * 5000,
    Umsatz_pro_5k = (Gesamtumsatz / Bevoelkerung) * 5000
  )

kable(analysis_data %>% 
        select(Stadt, Temperatur, Bevoelkerung,
               Verkaeufe_pro_5k, Umsatz_pro_5k) %>%
        arrange(desc(Verkaeufe_pro_5k)), 
      digits = 2,
      format.args = list(big.mark = "'"),
      caption = "Normalisierte Verkaufsdaten (pro 5,000 Einwohner)")
```

```{r plot-normalized, fig.width=10, fig.height=6}
# Normalisierte Verkäufe vs. Temperatur
p2 <- ggplot(analysis_data, aes(x = Temperatur, y = Verkaeufe_pro_5k)) +
  ...
  labs(
    ...
    subtitle = "Verkäufe pro 5,000 Einwohner",
    x = "Durchschnittstemperatur des wärmsten Monats (°C)",
    y = "Verkäufe pro 5,000 Einwohner",
    color = "Stadt"
  ) +
  theme_minimal() +
  theme(legend.position = "bottom")
...
Dieser Vergleich zeigt, warum die Normalisierung wichtig ist:

```{r comparison-plot, fig.width=12, fig.height=5}
grid.arrange(p1 + theme(legend.position = "none") + 
               ggtitle("Absolute Verkäufe (verzerrt durch Stadtgröße)"), 
             p2 + theme(legend.position = "none") +
               ggtitle("Normalisierte Verkäufe pro 5,000 Einwohner (fairer Vergleich)"), 
             ncol = 2)
```

**Interpretation**: Die normalisierten Werte (rechts) ermöglichen einen fairen Vergleich zwischen Städten unterschiedlicher Größe.
...
cor_normalized <- cor.test(analysis_data$Temperatur, 
                           analysis_data$Verkaeufe_pro_5k, 
                           method = "pearson")
...
cat("Normalisierte Verkäufe (pro 5,000 Einwohner) vs. Temperatur:\n")
...
model <- lm(Verkaeufe_pro_5k ~ Temperatur, data = analysis_data)
...
cor_data <- analysis_data %>%
  select(Temperatur, Bevoelkerung, 
         Verkaeufe_pro_5k, Durchschnittspreis)
...
ranking <- analysis_data %>%
  mutate(
    ...
    Rang_Normalisierte_Verkaeufe = rank(-Verkaeufe_pro_5k)
  ) %>%
  select(Stadt, Temperatur, Rang_Temperatur, 
         Verkaeufe_pro_5k, Rang_Normalisierte_Verkaeufe) %>%
  arrange(Rang_Temperatur)

kable(ranking, 
      digits = 2,
      format.args = list(big.mark = "'"),
      col.names = c("Stadt", "Temperatur (°C)", "Rang Temp.", 
                    "Verkäufe pro 5k", "Rang Verkäufe"),
      caption = "Stadt-Ranking nach Temperatur und normalisierten Verkäufen")
```
...
region_summary <- analysis_data %>%
  ...
  summarise(
    ...
    Durchschnitt_Verkaeufe_pro_5k = mean(Verkaeufe_pro_5k),
    .groups = 'drop'
  )

kable(region_summary, 
      digits = 2,
      col.names = c("Region", "Anzahl Städte", "Ø Temperatur (°C)", 
                    "Ø Verkäufe pro 5k"),
...
cat(sprintf("Lugano: %.2f Verkäufe pro 5,000 Einwohner\n", 
            lugano_data$Verkaeufe_pro_5k))
cat(sprintf("Durchschnitt andere Städte: %.2f Verkäufe pro 5,000 Einwohner\n", 
            mean(other_data$Verkaeufe_pro_5k)))
cat(sprintf("Differenz: %.2f (%.1f%% %s)\n", 
            lugano_data$Verkaeufe_pro_5k - mean(other_data$Verkaeufe_pro_5k),
            abs((lugano_data$Verkaeufe_pro_5k / mean(other_data$Verkaeufe_pro_5k) - 1) * 100),
            ifelse(lugano_data$Verkaeufe_pro_5k > mean(other_data$Verkaeufe_pro_5k), "höher", "niedriger")))
```
...
cat(sprintf("   - Verkäufe pro 5k Einwohner: %.2f\n", 
            analysis_data %>% filter(Stadt == "Lugano") %>% pull(Verkaeufe_pro_5k)))

cat(sprintf("\n3. St. Gallen (kühlste Stadt, 19.6°C):\n"))
cat(sprintf("   - Verkäufe pro 5k Einwohner: %.2f\n", 
            analysis_data %>% filter(Stadt == "St. Gallen") %>% pull(Verkaeufe_pro_5k)))
